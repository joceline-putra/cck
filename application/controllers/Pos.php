...
class Pos extends MY_Controller{
    ...
    function __construct(){
        ...
        $this->contact_1_alias  = 'Pelanggan';
        $this->contact_2_alias  = 'Waitress';
        $this->ref_alias        = 'Meja';         
        $this->order_alias      = 'Order';
        $this->trans_alias      = 'Transaksi';
    ...
    function index(){
        ...
        if ($this->input->post()) {    
                ...
                case "payment_create":
                    ...
                    if ($this->form_validation->run() == FALSE){
                        ...
                        if($payment_method == 1){ //Cash
                            $set_account_id = $datas['account_cash'];
                        }else if($payment_method == 2){ //Transfer
                            $set_account_id = $datas['account_transfer'];                        
                            ...
                            $set_account_id = $datas['account_edc'];
                            ...
                            $set_account_id = $datas['account_free'];
                        }else if($payment_method == 5){ //QRIS
                            $set_account_id = $datas['account_qris'];
            ...
            */
            ...
            $data['whatsapp_config']  = 1;
            $data['title'] = 'POS';
            $data['identity'] = 222;
            $data['_view'] = 'layouts/admin/menu/sales/pos/pos_v2';
            // var_dump($data['products']);die;
            $this->load->view('layouts/admin/index',$data);
            $this->load->view('layouts/admin/menu/sales/pos/pos_v2_js.php',$data);      
        }
    }
    function rawbt(){
        // $this->load->view('layouts/admin/index',$data);
