...
$(document).ready(function(){
  ...
  var konten = '';
      konten += '- Post Order Transaksi manage <b>load-order</b> pending';
  // $.confirm({
  //   title: 'Info',
  //   content: konten,
  ...
  $(".nav-tabs").find('li[data-name="checkup/medicine"]').addClass('active');
...
$(function() {
  ...
  $("#start, #end").datepicker({
    ...
    todayHighlight: true,
    weekStart: 1 
  }).on('change', function(){ index.ajax.reload(); }); 
});
...
$(document).on("change","#syarat_pembayaran",function(e) {
  ...
  var index = $("#table-data").DataTable({
    ...
    "columnDefs": [
      ...
      {"targets":1, "title":"Poli","searchable":true,"orderable":true},
      {"targets":2, "title":"Nama Pasien","searchable":true,"orderable":true},      
  ...
  $(document).on("click","#btn-save",function(e) {
    ...
    if(next==true){
      ...
      var prepare = {
        ...
        ref: $("select[id='ref']").find(':selected').val(),        
      }
    ...
    $.ajax({
      ...
      success:function(d){
        if(parseInt(d.status)==1){ /* Success Message */
          ...
          $("select[name='ref']").append(''+
                            '<option value="'+d.result.ref_id+'">'+
                              d.result.ref_name+
                            '</option>');
          $("select[name='ref']").val(d.result.ref_id).trigger('change');          
